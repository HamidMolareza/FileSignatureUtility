...
namespace FileSignatureValidation
{
    public class Detection
    {
        ...
        public Task<MethodResult<string?>>
            DetectFileTypeAsync(string fileName, int maximumReadBytes = MaximumReadBytes) =>
            TryExtensions.TryAsync(() => ReadBytesAsync(fileName, maximumReadBytes))
                .OnSuccessAsync(bytes => _fileSignatureValidation.FindSignature(bytes));

        public static string? DetectFileType(IEnumerable<byte> fileSignature, IEnumerable<FileType> types)
        {
            foreach (var type in types)
            {
                var isAny = type.Signatures.Any(typeSignature => fileSignature.Take(typeSignature.Length)
                    .SequenceEqual(typeSignature));
                if (isAny)
        ...
        private static int GetMaximumSignaturesLength(IEnumerable<FileType> fileTypes) =>
            fileTypes.Aggregate((f1, f2) =>
                f1.Signatures.Count > f2.Signatures.Count ? f1 : f2).Signatures.Count;
